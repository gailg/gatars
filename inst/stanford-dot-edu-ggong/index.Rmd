...
---
title: "gstir Version-0.1.5 016-10-26 08:28:28 PDT"
author: ''
output:
  html_document:
...
The package `gritsr3` provides an example data set which I will use to illustrate how to use the package.
...
## `hotspot` is available as soon as you enter `gritsr3`:
...
# gritsr-sampling-set {#gritsr-sampling-set}

Once you have your data in R, and you have checked that your genotype matrix has full rank, use the function `gritsr_sampling_set`  to create your sampling sets. 

## A description of how `gritsr_sampling_set` builds the sampling sets
...
$p_m \times \big[1 -$ `epsilon_on_log_scale` $, 1 +$ `epsilon_on_log_scale` $\big]$.  If the number of markers satisfying the matching requirement exceeds `1000`, `gritsr` randomly chooses `1000`.


## The arguments of `gritsr_sampling_set`
...
## Example calls to `gritsr_sampling_set` 
...
## A description of `gritsr`

`gritsr` calculates the p-values of the following seven statistics: the squared burden $Q_B$, the SKAT $Q_S$, the case based $Q_C$, and optimal linear combinations $Q_{BS}$, $Q_{BC}$, $Q_{SC}$, $Q_{BSC}$. 
...
This object is the result of `gritsr_sampling_set` and you may follow the example from [gritsr-sampling-set](#gritsr-sampling-set)
...
start = Sys.time()
set.seed(1)
gritsr(fam, Psi, sampling_set, N_sim_reps = 1000, weights = c(5, 3, 2))
