...
The significance level $Q(\alpha)$ for any given $\alpha$ can be calculated using the `davies` function from the `CompQuadForm` package. Such $Q(\alpha)$ include the three basic statistics.  However because the optimized statistics depend on data-driven $\alpha$'s, `davies` significance levels cannot be used for these statistics.  `gatars` estimates the significance levels of the optimized statistics using *genome resampling*.

To estimate the limiting null distribution of an optimized $Q$-statistic, `gatars` simulates  many null versions of the $Q$-statistic corresponding to a large number $K$ of simulated null genotype matrices $\tilde{G}^{(1)}, \cdots, \tilde{G}^{(K)}$.  Specifically, for each $k$, construct the $N \times M$ matrix $\tilde{G}^{(k)}$ by sampling one column from each *sampling set*, described below.  Then, for the $k$-th simulated matrix $\tilde{G}^{(k)}$, find the linear combination $\alpha^{k}$  having minimal nominal significance level $P_{\alpha^{k}}$  and store the transformed value $X_{\alpha^{k}} = - \text{log}_{10} \big( P_{\alpha^{k}} \big)$ . Repeat this procedure $K$ times and estimate the significance level of the observed $Q$ statistic to be the proportion of the $K$ transformed values $\Big\{X_{\alpha^k} \Big\}_{k = 1, \cdots, K}$ that exceed the observed transformed P-value corresponding to the observed $Q$ statistic.

Given user-supplied genotypes for each subject at roughly 100,000 markers located throughout the autosomal genome, `gatars` creates $M$ sampling sets, one for each target marker, with the sampling set for the $m$-th target marker containing markers all of whose minor allele frequencies (MAFs) match that of that target marker.
`gatars` also insures that the markers in the sampling sets are independent of target markers and markers supplied by the user that are known to be trait-associated. 
